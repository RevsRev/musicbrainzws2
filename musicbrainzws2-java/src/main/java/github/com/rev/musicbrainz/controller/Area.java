...
import github.com.rev.musicbrainz.query.browse.ArtistBrowseWs2;
import github.com.rev.musicbrainz.query.browse.LabelBrowseWs2;
import github.com.rev.musicbrainz.query.browse.PlaceBrowseWs2;
import github.com.rev.musicbrainz.query.browse.ReleaseBrowseWs2;
import github.com.rev.musicbrainz.query.lookUp.LookUpWs2;
import github.com.rev.musicbrainz.query.search.AreaSearchWs2;
...
// -------------- Search  -------------------------------------------------//
    ...
    public AreaWs2 getComplete(String id) throws MBWS2Exception {
        ...
        setEntity(lookUp(id));
        if (getIncludes().isArtists()) {
            getFullArtistList();
        }
        if (getIncludes().isLabels()) {
            getFullLabelList();
        }
        if (getIncludes().isReleases()) {
            getFullReleaseList();
        }
        if (getIncludes().isPlaces()) {
            getFullPlaceList();
        }
    ...
    public List<ArtistWs2> getNextArtistListPage() {
        ...
        if (!hasMoreArtists()) {
            return new ArrayList<ArtistWs2>();
        }
