...
import github.com.rev.musicbrainz.controller.Place;
...
public class RecordingWs2 extends EntityWs2 {
    ...
    public String getRecordingPlaceDisplay() {
        ...
        for (Iterator<RelationWs2> i = getRelationList().getRelations().iterator(); i.hasNext(); ) {
            ...
            if (!r.getTargetType().equals(RelationWs2.TO_PLACE)) {
                continue;
            }
            if (!r.getType().equals(RelationWs2.RECORDEDAT)) {
                continue;
            }
            ...
            if (r.getBeginDate().isEmpty() && r.getEndDate().isEmpty()) {
                date = "";
            } else if (r.getBeginDate().isEmpty()) {
                date = r.getBeginDate();
            } else if (r.getEndDate().isEmpty()) {
                date = r.getEndDate();
            } else if (r.getBeginDate().equals(r.getEndDate())) {
                date = r.getEndDate();
            } else {
                date = r.getBeginDate() + " - " + r.getEndDate();
            }
            if (!date.isEmpty()) {
                date = "(" + date + ")";
            }

            if (!plstr.isEmpty() && !date.isEmpty()) {
                plstr = plstr + " " + date;
            } else if (plstr.isEmpty()) {
                plstr = date;
            }
    ...
    public String getMixingPlaceDisplay() {
        ...
        for (Iterator<RelationWs2> i = getRelationList().getRelations().iterator(); i.hasNext(); ) {
            ...
            if (!r.getType().equals(RelationWs2.MIXEDAT)) {
                continue;
            }
    ...
    public boolean equals(Object object) {
        if (!(object instanceof RecordingWs2 other)) {
            return false;
        }
